...
void InorderTreePrintFile(rb_red_blk_tree* tree, rb_red_blk_node* x, FILE* fp) {
    rb_red_blk_node* nil=tree->nil;
    rb_red_blk_node* root=tree->root;
    if (x != tree->nil) {
      InorderTreePrintFile(tree, x->left, fp);
      tree->PrintKeyFile(x->key, fp);
      InorderTreePrintFile(tree, x->right, fp);
    }
}
...
void RBTreePrintFile(rb_red_blk_tree* tree, FILE* fp) {
